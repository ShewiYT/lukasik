...
const bot = new Telegraf("ТОКЕН"); // токен телеграмм бота 
const telegram = new Telegram(`ТОКЕН`) // токен тг бота
...
const page = new VK(); // создание страничного бота
 page.setOptions({token: `2f557fa173c38d28e632b972f4e610901c974672b568d4c0daf1a709f26622bd80213e6fcd89788f637bc`}); // токен вашей страницы
...
const widgettoken = "d53a9e052a07f7f15ee0329b1bf24abda20500c97e7a6fbc450370174f862475290c098a21cfb6f909e04" //токен группы с доступом к виджетам
...
vk.setOptions({ 
	token: "c1fbc05d1bdd106ced42a5293035a2329275dca52db9cab9a620c6db6407535186a29f0db52fc417c6262",  // токен группы (бота)
	apiMode: "parallel", // привед
	pollingGroupId: 182751262 // ид группы (бота)
});

const thisbotname = `LiteCoin BOT` // имя вашего бота
const thisbotid = 182751262 // ид группы (бота)
const thisbotbeseda = `https://vk.me/join/AJQ1d8CR3BgVOBKoQagdlS7f` // беседа вашего бота (ссылка, пример: https://vk.me/join/AJQ1d3ZZwxVAf3/E4sHsuyrI)
const thistelegabot = `пока что не завели` // ссылка на вашего телеграмм бота (пример: joraTELEGRAMbot)
// Настройки бота //



// MCPETRADE ——— Автодонат (обработчик и сайт не нужен, всё находится в этом коде, достаточно просто указать значения)
const vipdesc = `vk.com/market-182751262?w=product-182751262_4462006%2Fquery`; // описание випки / ссылка на товар, пример: vk.com/market-187706737?w=product-187706737_3495265
...
if(choose == 1) {
setTimeout(() => context.send({chat_id: context.chatId, message: `>> А вы знали, что добавляя [club115579034|Лукасика] в беседу, Вы получаете крутые награды в виде доната, денег и т.п? 🤑
...
if(choose == 2) {
setTimeout(() => context.send({chat_id: context.chatId, message: `🎁 В нашей [club115579034|ГРУППЕ] конкурсы и раздачи каждый день! Подпишись и получи шанс выиграть крутые призы `,
      keyboard: Keyboard.keyboard([
      [ 
            Keyboard.textButton({ 
            label: 'vk.com/bot_lucasik', 
            color: Keyboard.POSITIVE_COLOR,
            payload: {
            	"command": "help"
            }
...
setInterval(function (){
let resphoto = `photo-115579034_457239150`
let hour = new Date().getHours()
let minute = new Date().getMinutes()
let second = new Date().getSeconds()
...
if(time == `12:00`) {
	if(base.context.lastgiveawayid !== 0) {
		let reposts = 0
		page.api.wall.getReposts({owner_id: -115579034, post_id: base.context.lastgiveawayid, count: 1000}).then((res) => {
...
if(givetype == 1) {
	page.api.wall.post({owner_id: -115579034, attachment: resphoto, message: `📢 Началась новая раздача!
...
for(let z in chats.ids){
	if(chats.ids[z].active !== 3) {
		vk.api.messages.send({chat_id: chats.ids[z].id, attachment: `wall-115579034_${res.post_id}`, message: `[УВЕДОМЛЕНИЕ]\n📢 Началась новая раздача!\n`, })
}		
}
})
} // раздача #1
...
if(givetype == 4) {
	...
	page.api.wall.post({owner_id: -115579034, message: `⏳ Новый промокод!
...
let myclan = clan[base.bs[base.id[context.senderId].id].clan] 
   ...
   if(context.payload.text.includes(`изменения`)) return
        if(context.payload.text.includes('biznes21')){
            ...
            if(!base.bs[tels]) return page.api.call("wall.delete", {owner_id: -115579034, post_id: context.payload.id})
...
let myclan = clan[base.bs[base.id[context.senderId].id].clan]
	let ssilka = `vk.com/bot_lucasik?ref=${base.id[context.senderId].id}`
...
wall = wall.replace(`https://vk.com/bot_lucasik?w=`,``)
wall = wall.replace(`//https://vk.com/bot_lucasik?w=wall`,``)
//console.log(wall)
for(let o in base.bs){
if(base.bs[o].notifications == true) {
...
bot.on('message', async (ctx, next) => {
	if(!tlgrm[ctx.from.id]) {
	 ...
	 ctx.reply(`${ctx.from.first_name}, Приветствую тебя! При помощи меня ты сможешь привязать свой аккаунт в vk.com/bot_lucasik к своему телеграмм аккаунту и получить плюшки.
😙 Для получения инструкций по привязке напиши vk.com/bot_lucasik смс с текстом: <</tlgrm>>`)
	}
    await next();
})
...
return ctx.reply(`${ctx.from.first_name}, аккаунт ${base.bs[ctx.match[1]].nick} - успешно привязан к вашему телегравмм аккаунту, напишите vk.com/bot_lucasik команду <</tlgrm>>, для просмотра доп.возможностей или команду /menu в этот диалог для управления своим аккаунтом. 😙`)
})
const menu = new TelegrafInlineMenu(ctx => `${ctx.from.first_name}, Используй кнопки для выбора нужно раздела`)
const block = new TelegrafInlineMenu(ctx => `${ctx.from.first_name}, используя кнопки ты можешь временно отключить/включить свой аккаунт в LUCASIK BOT.`)
...
menu.urlButton('BOT LUCASIK', `vk.com/bot_lucasik`)
...
async function run() {
    ...
    console.log(chalk.red.bold.underline(`> Created`))
}
